...
// loading实例列表
let instanceofLoading: any[] = [];
export const startLoading = () => {
  instanceofLoading.push(Toast({ type: 'loading', message: '加载中...', loadingType: 'spinner', duration: 0 }));
};
// 关闭loading 遍历实例清除
export const closeLoading = () => instanceofLoading.map((v) => v.clear());
